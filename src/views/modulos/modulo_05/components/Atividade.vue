    ...
    <v-row v-else class="completed-container" justify="center">
      <v-col cols="12" sm="4">
        ...
        <Paragraph class="completed-score mb-5">
          Você acertou <span v-if="score < 1">apenas </span>
          <strong
            ><u>{{ score }}</u></strong
          >
...
import Questoes from "@/data/questoes/modulo_05.json";
  ...
  methods: {
    ...
    checkAnswer() {
      ...
      if (this.questionIndex === this.questions.length - 1) {
        this.completed = true;
        if (this.score >= 1 && !this.$store.state.progresso_modulos.modulo_05) {
          this.$store.state.progresso_modulos.modulo_05 = true;
